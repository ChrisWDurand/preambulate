...
The `install` command merges `SessionStart`, `PostToolUse`, and `Stop` hooks into your global Claude Code settings. Safe to run more than once — commands already present are skipped.
...
Hooks fire automatically when installed via `preambulate install`:
...
| `SessionStart` | Session open | `preambulate sync pull` | Pulls remote graph into local (no-op if `PREAMBULATE_API_KEY` unset) |
...
| `Stop` | Each response | `preambulate sync push` | Pushes full local graph to remote (no-op if `PREAMBULATE_API_KEY` unset) |
...
- `GOVERNS` — the structure of the source shapes how the target is traversed
...
Defined in `schema.cypher` (DDL) and `schema.spec.md` (spec). Node types: `Seed`, `Concept`, `Artifact`, `Cluster`, `Context`, `Observation`, `Decision`. Relationship types use `CREATE REL TABLE GROUP` because most support multiple FROM-TO node type pairs.
...
| `preambulate/graph.py` | GraphConnection abstraction — only file that imports kuzu |
...
| `preambulate/infer.py` | PostToolUse hook — infers DERIVES_FROM edges from Python imports and call graph |
| `preambulate/cluster.py` | Community detection — Phase A (file-level) and Phase B (symbol-level) |
...
| `preambulate/sync.py` | Sync command — push/pull graph snapshot to/from remote |
| `preambulate/export.py` | Migration tool — dump/restore full graph to/from JSON |
| `preambulate/keystore.py` | Encryption key management — generate, load, rotate |
| `preambulate/identity.py` | Machine ID — stable identifier for the local project instance |
| `preambulate/sync_state.py` | Sync checkpoint — tracks last push/pull timestamp |
| `interface/` | Interface contracts — specs for external integrations |
